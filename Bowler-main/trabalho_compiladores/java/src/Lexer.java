...
public class Lexer {
    ...
    private void scanToken() {
        ...
        switch (c) {
            ...
            case '+': add(match('=') ? TokenType.PLUS_EQUAL : TokenType.PLUS); break;
            case '-': add(match('=') ? TokenType.MINUS_EQUAL : TokenType.MINUS); break;
            ...
            case '&': if (match('&')) add(TokenType.AND_AND); else error("Esperado '&' para '&&'."); break;
            case '|': if (match('|')) add(TokenType.OR_OR); else error("Esperado '|' para '||'."); break;
            case ' ': case '\r': case '\t': break; // ignorar espaços
            case '\n': line++; col = 1; break;
            case '"':
    ...
    private void interpolatedString() {
        ...
        while (!isAtEnd()) {
            ...
            if (c == '"') {
                ...
                // Após o parser consumir a expressão e o RIGHT_BRACE, o scanToken será chamado novamente
                return;
            } else if (c == '}') {
                if (sb.length() > 0) {
                    add(TokenType.STRING, sb.toString());
    ...
    private void charLiteral() {
        if (isAtEnd()) error("Char não terminado.");
        char value = advance();
        if (peek() != '\'') error("Char deve ter apenas um caractere e ser fechado com aspas simples.");
        advance(); // fecha aspas
    ...
    private char advance() { char c = src.charAt(current++); col++; return c; }
    ...
    private void error(String message) { throw new RuntimeException("[Linha " + line + ", Col " + col + "] " + message); }
}
