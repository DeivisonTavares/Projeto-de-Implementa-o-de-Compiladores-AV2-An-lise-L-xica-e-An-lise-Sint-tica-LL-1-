...
/**
 * LL1GrammarAnalyzer - Calcula conjuntos FIRST e FOLLOW para uma gramática LL(1)
 * 
...
public class LL1GrammarAnalyzer {
    ...
    private Set<String> firstOf(List<String> symbols) {
        ...
        // Se todos derivam ε
        if (symbols.stream().allMatch(s -> 
            nonTerminals.contains(s) && first.get(s).contains("ε") || 
            s.equals("ε"))) {
            result.add("ε");
        }
                    ...
                    if (!terminal.equals("ε")) {
                        ...
                        if (existing != null && !existing.equals(production)) {
                            ...
                            if (existingIsEps && !newIsEps) {
                                ...
                                conflicts.add("RESOLVIDO (pref. nao-eps): M[" + nt + ", " + terminal + "] => " + production + " sobre " + existing);
                            } else if (!existingIsEps && newIsEps) {
                                // manter existing (não sobrescrever)
                                conflicts.add("IGNORADO (eps vs nao-eps): M[" + nt + ", " + terminal + "] manter " + existing + " (ignorando " + production + ")");
                            } else {
                                String msg = "CONFLITO LL(1) em M[" + nt + ", " + terminal + "] entre " + existing + " e " + production;
                    ...
                    for (String followTerm : follow.get(nt)) {
                        ...
                        if (existing != null && !existing.equals(production)) {
                            ...
                            if (existingIsEps && !newIsEps) {
                                ...
                                conflicts.add("RESOLVIDO (pref. nao-eps): M[" + nt + ", " + followTerm + "] => " + production + " sobre " + existing);
                            } else if (!existingIsEps && newIsEps) {
                                conflicts.add("IGNORADO (eps vs nao-eps): M[" + nt + ", " + followTerm + "] manter " + existing + " (ignorando " + production + ")");
                            } else {
                                String msg = "CONFLITO LL(1) em M[" + nt + ", " + followTerm + "] entre " + existing + " e " + production;
                                System.err.println(msg);
                                conflicts.add(msg);
                            }
                        } else {
